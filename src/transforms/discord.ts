...
export function normalizeMarkdownHeadings(text: string): string {
  ...
  if (!matches.some((m) => m[1].length > 3)) return text;

  const offset = Math.min(...matches.map((m) => m[1].length)) - 1;
  return text.replace(HEADING_RE, (_match, hashes: string) => {
    const newLevel = Math.min(hashes.length - offset, 3);
...
export function stripInlineCodeInMarkdownTables(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fenceMatch) {
      const fullMarker = fenceMatch[1];
      if (fenceMarker === null) {
        fenceMarker = fullMarker;
      } else if (
        fenceMarker[0] === fullMarker[0] &&
        fullMarker.length >= fenceMarker.length
      ) {
        fenceMarker = null;
      }
      result.push(line);
